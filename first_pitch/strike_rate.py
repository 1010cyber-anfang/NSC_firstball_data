...
events  = load()
n       = len(events)
strikes = sum(1 for *_, t in events if t in STRIKE)
noswing = [t for *_, t in events if t not in SWING]
ns_strk = sum(1 for t in noswing if t == "0S")

print(f"Total first pitches      : {n}")
print(f"First-pitch strike rate  : {pct(strikes, n)}  ({strikes}/{n})")
print(f"Strike rate on takes     : {pct(ns_strk, len(noswing))}  ({ns_strk}/{len(noswing)})")