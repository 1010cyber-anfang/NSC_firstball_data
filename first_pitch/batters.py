...
rows = [(b, d) for b, d in agg.items() if d["n"] >= MIN_N]
rows.sort(key=lambda x: -x[1]["swing"] / x[1]["n"])
print(f"{'Team':<6} {'Batter':<12} {'n':>4} {'Swing%':>8} {'OB%':>7} {'OB':>4}")
print("-" * 48)
for (team, name), d in rows:
    print(f"{team:<6} {name:<12} {d['n']:>4} {pct(d['swing'], d['n']):>8} "
